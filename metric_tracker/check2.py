...
cap = cv2.VideoCapture('D:\Program Files\metric_tracker\crops_raw/{}.avi'.format(video_id))
anno_file = open('D:\Program Files\metric_tracker\crop_gt/new_video_{}_0.txt'.format(video_id))
annotations = anno_file.readlines()
annotations = [list(map(float, annotation.split())) for annotation in annotations]

anno_file_test = open('D:\Program Files\metric_tracker\label/IR_{}.txt'.format(video_id))
...
while (g_i < len(annotations) - 1):
    ...
    cv2.rectangle(frame, (tposition[0], tposition[1]), (tposition[0] + tposition[2], tposition[1] + tposition[3]),
                  (0, 255, 0), 2)
